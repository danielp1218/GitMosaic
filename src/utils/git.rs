...
use chrono::{DateTime, FixedOffset, Days};
...
pub fn get_max_daily_contributions(year: i32) -> u32 {
    ...
    let output = Command::new("gh")
        ...
        .stdout(Stdio::piped())
        .spawn().expect("Failed to execute gh command")
        .wait_with_output().expect("Failed to read gh output");

    let stdout = String::from_utf8_lossy(&output.stdout);
    
    // The output is expected to be a JSON object like: {"contributionCount": X }
    let stdout = stdout.trim().replace("{", "").replace("}", "").replace("\"", "");
