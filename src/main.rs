...
fn main() {
    ...
    loop {
        ...
        let continue_input = get_input(
            &mut rl,
            "Do you want to proceed with this image? (y/n): ",
            |input| input.eq_ignore_ascii_case("y") || input.eq_ignore_ascii_case("n"),
    ...
    let repo_name = get_input(
        &mut rl,
        "Enter the repository name to create: ",
        |input| !input.is_empty(),
        "Invalid repository name. Please enter a valid name.",
    );
    let local_path = get_input(
        &mut rl,
        "Enter the local path to create the repository in: ",
        |input| !input.is_empty(),
        "Invalid local path. Please enter a valid path.",
    );
    let remote_url = get_input(
        &mut rl,
        "Enter the remote URL (leave blank to create a new GitHub repo): ",
        |_| true,
        "",
    );
    utils::git::setup_git_repo(&repo_name, &local_path);

    let year_input = get_input(
        &mut rl,
        "Enter the year to insert the drawing (e.g., 2023): ",
        |input| input.parse::<i32>().is_ok() && input.parse::<i32>().unwrap() > 1970,
    ...
    println!(
        "Max daily contributions for year {}: {}",
        year, max_contributions
    );
    let mult = std::cmp::max(max_contributions / 4, 1);
    println!("Using contribution multiplier: {}", mult);

    let commits_needed: u32 = quantized_image
        .iter()
        .flatten()
        .map(|&lvl| lvl as u32 * mult)
        .sum::<u32>()
        * mult;
    println!("Total commits needed: {}", commits_needed);

    let h_offset = get_input(
        &mut rl,
        "Enter horizontal offset (number of weeks to shift right): ",
        |input| input.parse::<i32>().is_ok(),
        "Invalid offset. Please enter a valid integer.",
    )
    .parse::<i32>()
    .unwrap();
    ...
    let start_date =
        DateTime::parse_from_rfc3339(&utils::git::create_date(year, 1, 1, 0, 0, 0)).unwrap();
    ...
    for x in 0..quantized_image[0].len() {
        for row in &quantized_image {
            ...
            current_date = utils::git::increment_date(current_date, 1);
        }
    }
